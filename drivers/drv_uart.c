...
void rt_hw_uart_init(void)
{
    ...
    serial8.ops    = &ls1c_uart_ops;
    serial8.config = config;

    pin_set_purpose(54, PIN_PURPOSE_OTHER);
    pin_set_purpose(55, PIN_PURPOSE_OTHER);
    pin_set_remap(54, PIN_REMAP_DEFAULT);
    pin_set_remap(55, PIN_REMAP_DEFAULT);

    rt_hw_interrupt_install(uart->IRQ, uart_irq_handler, &serial8, "UART8");

    /* register UART1 device */
    rt_hw_serial_register(&serial8,
                          "uart8",
                          //RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_DMA_RX,
                          RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX,
                          uart);
